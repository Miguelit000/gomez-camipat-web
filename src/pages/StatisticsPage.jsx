...
export default function StatisticsPage() {
  const { portfolioId, userRole } = useContext(AuthContext); 
  ...
  const getSqnColor = (sqn) => {
    if (sqn >= 3) return '#10b981'; 
    if (sqn >= 2) return '#3b82f6'; 
    if (sqn >= 1) return '#f59e0b'; 
    return '#ef4444'; 
  };
  ...
  return (
    <div style={{ padding: '40px', fontFamily: 'sans-serif', maxWidth: '1200px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: '20px', marginBottom: '40px' }}>
        ...
        <StatCard title="Expectancy ($)" value={`$${formatMoney(stats.expectancyUsd)}`} subtitle="Esperanza matemática promedio" valueColor={stats.expectancyUsd > 0 ? '#10b981' : '#ef4444'} />
        
        <StatCard title="Risk / Reward ($)" value={`1 : ${stats.averageRewardToRiskUsd}`} subtitle="Ratio de Riesgo/Beneficio real" />

        <StatCard title="Max Drawdown" value={`-$${formatMoney(stats.maxDrawdownUsd)}`} subtitle={`${stats.maxDrawdownPct}% de caída máxima`} valueColor="#ef4444" />
        
        <StatCard title="Average Drawdown" value={`-$${formatMoney(stats.averageDrawdownUsd)}`} subtitle="Promedio de caídas" valueColor="#f97316" />
